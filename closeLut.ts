...
const closeLut = async () => {
  try {
    ...
    if (lutData.length == 0) {
      log.error("No lut data saved as file")
      return
    }
    ...
    try {
      ...
      log.info("Cool Down sig:", coolDownSig)

    } catch (error) {
      log.error("Deactivating LUT error:", error)
    }
    ...
    try {
      ...
      log.info("Close LUT Sig:", closeSig)
    } catch (error) {
      log.error("Close LUT error:", error)
    }
  } catch (error) {
    log.error("Unexpected error while closing the LUT")
  }
}
