  ...
  wallets.map(async (kp, i) => {
    try {
      ...
      if (tokenAccounts.value.length > 0)
        ...
        log.error("No token accounts found")
      ...
      for (let j = 0; j < accounts.length; j++) {
        ...
        while (true) {
          if (i > 10) {
            log.error("Sell error before gather")
            break
          }
          ...
          try {
            log.info("Selling token:", accounts[j].accountInfo.mint.toBase58())
            const sellTx = await getSellTxWithJupiter(kp, accounts[j].accountInfo.mint, tokenBalance.amount)
            if (sellTx == null) {
              // log.error(`Error getting sell transaction`)
              throw new Error("Error getting sell tx")
            }
            // log.info(await solanaConnection.simulateTransaction(sellTx))
            ...
            log.info("Sold token, ", tokenSellTx)
            break
          } catch (error) {
            i++
          }
        ...
        log.info("Wallet address & balance : ", kp.publicKey.toBase58(), tokenBalanceAfterSell.amount)
      ...
      if (ixs.length) {
        ...
        log.info(`Closed and gathered SOL from wallets ${i} : https://solscan.io/tx/${sig}`)
        return
      }
    } catch (error) {
      log.error("transaction error while gathering", error)
      return
    }
  })
}
