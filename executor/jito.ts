...
export const executeJitoTx = async (transactions: VersionedTransaction[], payer: Keypair, commitment: Commitment) => {
  ...
  try {
    ...
    log.info('Sending transactions to endpoints...');
    ...
    if (successfulResults.length > 0) {
      ...
      log.info("Wallets bought the token plz check keypairs in the data.json file in key folder")
      ...
      log.error(`No successful responses received for jito`);
    }
    return null
  } catch (error) {
    log.error('Error during transaction execution', error);
    return null
  }
}
