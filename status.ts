...
export const displayStatus = async () => {
  ...
  try {
    ...
    wallets.map(async (kp, i) => {
      ...
      if (!tokenBalance) {
        log.error("Token balance not retrieved, Error...")
        return
      }
      ...
      log.info("Wallet ", i, " : ", kp.publicKey.toBase58(), ", Holding Percent -> ", percent, "%, Token Balance -> ", tokenBalance.toFixed(2))
    })
  } catch (error) {
    log.error("Error in displaying wallets status")
    return
  }
}
