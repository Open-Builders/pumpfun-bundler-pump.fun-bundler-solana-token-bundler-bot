...
export const getUploadedMetadataURI = async (): Promise<string> => {
  ...
  try {
    ...
    log.info('Image uploaded to IPFS:', imageUploadResult);
    log.info('IPFS URL:', `https://cf-ipfs.com/ipfs/${imageUploadResult.cid}`);
    ...
    log.info('File uploaded to IPFS:', metadataUploadResult);
    log.info('IPFS URL:', `https://cf-ipfs.com/ipfs/${metadataUploadResult.cid}`)
