...
export async function sendTx(
  ...
  try {
    log.info((await connection.simulateTransaction(versionedTx, undefined)))

    const sig = await connection.sendTransaction(versionedTx, {
      skipPreflight: false,
    });
    log.info("Transaction signature: ", `https://solscan.io/tx/${sig}`);
...
export const readBuyerWallet = (fileName: string) => {
  ...
  try {
    ...
    log.error('Error reading public key from file:', error);
...
export const printSPLBalance = async (
  ...
  if (balance === null) {
    ...
    log.info(`${info ? info + " " : ""}${user.toBase58()}:`, balance);
  }
};
...
export const execute = async (transaction: VersionedTransaction, latestBlockhash: Blockhash, isBuy: boolean | 1 = true) => {
  ...
  if (confirmation.value.err) {
    ...
    if(isBuy === 1){
      ...
      log.info(`Success in buy transaction: https://solscan.io/tx/${signature}`)
    else
      log.info(`Success in Sell transaction: https://solscan.io/tx/${signature}`)
  }
  return signature
}
  ...
  try {
    ...
    try {
      if (fs.existsSync(filePath)) {
        ...
        log.info(`File ${filePath} deleted and create new file.`);
      }
      fs.writeFileSync(filePath, JSON.stringify(newData, null, 2));
      log.info("File is saved successfully.")
    } catch (error) {
      log.error('Error saving data to JSON file:', error);
    }
  }
};
...
export async function newSendToken(
  ...
  try {
      ...
      if (tokensToSendArr.length !== walletKeypairs.length) {
          log.error("Number of wallets and token amounts array is not matching")
          throw new Error("Number of wallets and token amounts array is not matching")
      }

      log.info("Token amount of the srcAta: ", (await connection.getTokenAccountBalance(srcAta)).value.amount)

      const insts: TransactionInstruction[] = []
      log.info("Wallet length to distribute: ", walletKeypairs.length)
      for (let i = 0; i < walletKeypairs.length; i++) {
          const destKp = walletKeypairs[i]
          const amount = tokensToSendArr[i]
          log.info("token amount ", amount)
      ...
      log.info("total number of instructions : ", insts.length)
      const txs = await makeTxs(insts, walletKeypair)
      if (!txs) {
          log.error("Transaction not retrieved from makeTxs function")
      ...
      try {
          ...
          log.error("Error in transaction confirmation part : ", error)
      }
  } catch (error) {
      log.error("New Send Token function error : ", error)
  }
}
...
const makeTxs = async (insts: TransactionInstruction[], mainKp: Keypair) => {
  try {
      ...
      for (let i = 0; i < txNum; i++) {
          ...
          log.info(await connection.simulateTransaction(tx))

          txs.push(tx)
      }
      if (txs.length == 0) {
          log.error("Empty instructions as input")
          throw new Error("Empty instructions as input")
      }
      return txs
  } catch (error) {
      log.error("MakeTxs ~ error")
  }
